...
//@desc Save a song
//@route put /api/songs/save
//@access private
const saveSong = async (req, res) => {
  try {
    const songExists = await Song.find({ title: req.body.title });
    if (songExists.length >= 1) {
      return res
        .status(400)
        .json({ message: "The song already exists in database." });
    }
    const songCreated = await Song.create(req.body);
    if (!songCreated) {
      return res.status(400).json({ message: "Song not saved." });
    }
    res.send("Song saved.");
  } catch (error) {
    return res.status(409).json({ message: error.message });
